...
const int WORKMAX = 256;
int WORKERS = 8;

int main (int argc, char** argv)
{
    if (argc > 1) WORKERS = strtod(argv[1], NULL);
...
long threaded_prime_count (long N)
{
    long total = 0, tmp[WORKMAX] = {0};         // Results from each worker
    thread tid[WORKMAX];
    for (int i=0; i<WORKERS; ++i)
    {
        ...
        tid[i] = thread(worker, Ni, fi|1, si, &tmp[i]);  // Pass output address
    }
    /* Wait for all threads to end */
    for (int i=0; i<WORKERS; ++i)
    {
        tid[i].join();
        total += tmp[i];                        // Add subtotal of each thread
    }
